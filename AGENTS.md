...
**Brain note (goals, backlog, full context):** [portfolio.md](file:///H:/My%20Drive/Brain2_backup/Projects/portfolio.md)
...
- Cache-bust JSON fetch calls (`projects.json` and `timeline.json`) by appending `?v=timestamp` or similar to prevent local caching issues.
...
- Hand-edit the Functional Prototypes count, Concepts count, or Last GitHub Update date in `index.html` — these are updated automatically by the Editor's publish workflow in `server.js`.
